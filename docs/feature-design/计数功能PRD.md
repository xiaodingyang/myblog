...
> 审查报告遗留 14 项 P1 + 10 项 P2，以下为首期纳入/推后决策。

### 首期纳入（P0 开发时同步完成）

| # | 审查编号 | 内容 | 原因 |
|---|---------|------|------|
| 1 | 1-1 | 平均停留极端值格式化 | 实现成本极低，避免线上异常显示 |
| 2 | 1-2 | 移动端数值字号降至 28px | 响应式适配顺手完成 |
| 3 | 3-2 | 表格路径 ellipsis + Tooltip | Ant Design 原生支持，1 行代码 |
| 4 | 5-2 | 移动端筛选输入框 width: 100% | 响应式适配顺手完成 |
| 5 | G-3 | 页面可见时 60s 静默刷新卡片 | 数据时效性基础需求 |
...
|---|---------|------|------|
| 6 | 1-3 | 趋势持平状态样式 | 边缘场景 |
| 7 | 1-4 | 时间切换叠加位移过渡 | 锦上添花 |
| 8 | 2-2 | 发光描边离屏 Canvas 优化 | P1 阶段性能调优 |
| 9 | 2-3 | 切换动画缩短至 800ms | 需实机体验后决策 |
| 10 | 2-4 | 触摸设备 Tooltip 固定显示 | 移动端优化 |
| 11 | 2-6 | 骨架屏与画线动画衔接 | 细节打磨 |
| 12 | 3-4 | 表格横向滚动渐变遮罩 | 移动端优化 |
| 13 | 4-1 | 饼图改用固定色板 | 需设计补充 |
| 14 | 4-2 | 移动端饼图响应式尺寸 | 移动端优化 |
| 15 | 4-3 | 中心文字大数缩写 | 边缘场景 |
| 16 | 4-4 | 扇区外扩动画时长 | 细节打磨 |
| 17 | 5-3 | 访问记录时间列排序 | 非核心功能 |
| 18 | 5-4 | 筛选条件 URL 同步 | 非核心功能 |
| 19 | G-2 | 超时阈值调整为 15s | 需线上数据验证 |
| 20 | G-4 | 深色模式适配 | 独立迭代 |
| 21 | 0-2 | 字体加载策略优化 | 性能迭代 |
| 22 | G-1 | 无障碍设计 | 推后 |
